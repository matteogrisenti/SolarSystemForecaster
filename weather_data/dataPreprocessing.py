...
def preprocess_hourly_data(input_csv, output_csv):
    ...
    try:
        ...
        # --- Step 4: Merge intra-hour duplicates ---
        df["hour"] = df["datetime"].dt.floor("h")
        df_hourly = df.groupby("hour")["value"].mean().reset_index()

        # --- Step 5: Detect missing or invalid hours ---
        ...
        # --- Step 7: Rename column dynamically ---
...
if __name__ == "__main__":
    # 📁 DEFINE DIRECTORIES
    raw_data_directory = Path("./raw_weather_csv")
    preprocessed_data_directory = Path("./preprocessed_weather_csv")
    ...
    if not csv_files:
        ...
        for file in csv_files:
            output_path = preprocessed_data_directory / f"{file.stem}_preprocessed.csv"
